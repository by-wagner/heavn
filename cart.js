...
function updateCart(product) {
  ...
  cartItems.forEach((item) => {
    ...
    listItem.innerHTML = `${item.name} <span class="badge badge-primary badge-pill">${item.quantity}</span>`;
  ...
  productlist.forEach((item) => {
    ...
    <div class="card card-shadow border-0" data-id="Deck-1">
        ...
        <div class="card-body">
            ...
            <button class="btn btn-dark btn-block add-to-cart" data-product='${JSON.stringify(
              item
            )}'>Add to Cart</button>
        </div>
    </div>
